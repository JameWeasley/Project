                ...
                if (err) {
                    ...
                    switch (req.method) {
                        case 'POST':
                            ...
                            await db_boss.collection.insertOne({ ...data, image: req.body.FI })
                            res.status(200).json({ msg: 'เพิ่มบอสหรือมอนส์เตอร์สำเร็จ' })
                            break
                        case 'DELETE':
                            if (isEmpty(req.query.id)) return res.status(500).json({ msg: 'ไม่พบบอสหรือมอนส์เตอร์' })
                            await db_boss.deleteOne({ _id: req.query.id })
                            res.status(200).json({ msg: 'ลบบอสหรือมอนส์เตอร์สำเร็จ' })
                            break
                        case 'PUT':
                            if (isEmpty(req.body.id)) return res.status(500).json({ msg: 'ไม่พบบอสหรือมอนส์เตอร์' })
                            await db_boss.findOneAndUpdate({ _id: req.body.id }, { name: req.body.FN, drop: req.body.IT, image: req.body.FI, maps: req.body.M });
                            res.status(200).json({ msg: 'แก้ไขบอสหรือมอนส์เตอร์สำเร็จ' })
                            break
                    }
